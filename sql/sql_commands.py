...
def show_career_stats(cursor):
    ...
    for name, team, career_length, games_played, hits in results:
        ...
        if career_length is not None:
            print(f"\nCareer Length: {career_length} years")
...
def show_stats_by_year_range(cursor):
    ...
    cursor.execute("""
        SELECT p.name, p.team, ps.statistic, ps.value, ps.year
        FROM Players p
    ...
    if results:
        current_year = None
        for name, team, stat, value, year in results:
            if year != current_year:
                current_year = year
                print(f"\nYear: {year}")
            print(f"{name} ({team}): {stat} = {value}")
    else:
